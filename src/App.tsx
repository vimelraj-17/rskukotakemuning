  ...
  return (
    <div className="site-shell">
      ...
      <main id="main-content">
        ...
        <section className="section" id="layouts" aria-labelledby="layouts-title"><div className="section-heading narrow"><p className="eyebrow">{copy.layouts.eyebrow}</p><h2 id="layouts-title">{copy.layouts.title}</h2><p>{copy.layouts.body}</p></div><div className="layout-grid">{propertyData.layouts.map((layout) => <article className="layout-card" key={layout.id}><div className="plan-scroll"><img src={assetUrl(layout.planAsset!)} alt={`Typical ${layout.sizeSqFt.toLocaleString('en-MY')} sq ft floor plan with three bedrooms and two bathrooms`} width="1600" height="1142" loading="lazy" decoding="async" /></div><div><span>{layout.name}</span><strong>{layout.sizeSqFt.toLocaleString('en-MY')} sq ft</strong><p>{layout.bedrooms} bedrooms · {layout.bathrooms} bathrooms</p></div></article>)}</div></section>
        ...
        <section className="section location-section" id="location" aria-labelledby="location-title"><div><p className="eyebrow">{copy.location.eyebrow}</p><h2 id="location-title">{copy.location.title}</h2><p>{copy.location.body}</p><address>{propertyData.location.address}</address><ul className="nearby-list">{propertyData.location.nearbyPlaces.map((place) => <li key={place.id}><span>{place.name}</span><strong>{place.distanceLabel}</strong></li>)}</ul></div><figure><div className="plan-scroll"><img src={assetUrl(copy.location.image)} alt={copy.location.imageAlt} width="1600" height="753" loading="lazy" decoding="async" /></div><figcaption>Indicative location diagram from the approved sales material.</figcaption></figure></section>
        ...
        <section className="contact-section" id="contact" aria-labelledby="contact-title"><p className="eyebrow eyebrow--light">{copy.contact.eyebrow}</p><h2 id="contact-title">{copy.contact.title}</h2><p>{copy.contact.body} Complete the guided selection above to generate an enquiry with the correct project details.</p><a className="button button--gold" href="#select-home">Start your selection</a></section>
      </main>
